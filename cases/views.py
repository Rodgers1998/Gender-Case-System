...
def case_create(request):
    """Automatically assign logged-in user to case."""
    if request.method == 'POST':
...
def case_update(request, pk):
    """Ensure user cannot overwrite ownership."""
    case = get_object_or_404(Case, pk=pk)
    if request.method == 'POST':
        form = CaseForm(request.POST, instance=case)
        if form.is_valid():
            updated_case = form.save(commit=False)
            updated_case.user = case.user  # Keep original owner
            updated_case.save()
...
def generate_case_pdf(request, case_id):
    """Generate a full case detail PDF with all important info."""
    ...
    p.drawCentredString(width / 2, height - 130, "Gender Department Case Report")
