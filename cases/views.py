...
def case_detail(request, pk):
    ...
    verify_url = request.build_absolute_uri(reverse('verify_case', args=[case.pk]))
    return render(request, 'cases/case_detail.html', {'case': case, 'verify_url': verify_url})
...
def generate_case_pdf(request, case_id):
    """Generate a full case detail PDF with QR code verification."""
    ...
    def draw(label, val):
        nonlocal y
        text = f"{label}: {val if val else 'N/A'}"
        p.drawString(border_margin + 20, y, text)
        y -= 14

    # Case details
    fields = [
        ("Case ID", case.case_id),
        ("Previous Case No.", case.previous_case_number),
        ("Assigned Officer", str(case.assigned_to or request.user.username)),
        ("Reporting Date", case.date_of_case_reporting),
        ("Intake Date", case.date_of_case_intake),
        ("Court Follow-up Date", case.date_of_court_followup),
        ("Stage of Case", case.stage_of_case_in_court),
        ("County", case.county),
        ("Sub-county", case.case_constituency_name),
        ("Ward", case.case_ward_name),
        ("Site", case.site),
        ("Assault Type", case.assault_type),
        ("Survivor Gender", case.survivor_gender),
        ("Age Group", case.age_group),
        ("Case Proceeding to Court", "Yes" if case.is_the_case_proceeding_to_court else "No"),
        ("Case Still in Court", "Yes" if case.case_still_in_court else "No"),
        ("Case Closed", "Yes" if case.case_is_closed else "No"),
        ("Referred to Police", "Yes" if case.referred_to_police else "No"),
    ...
    qr_buffer.seek(0)
    qr_image = ImageReader(qr_buffer)  # <-- Convert BytesIO to ImageReader
    p.drawImage(qr_image, width - 160, border_margin + 30, width=120, height=120)

    # Footer
    y -= 30
    p.drawString(border_margin + 20, y, f"Generated by: {request.user.username}")
    p.drawString(width - border_margin - 200, y, "Sign: ______________________")
    y -= 50
    p.drawString(width - border_margin - 200, y, "Stamp: ___________________")
...
@login_required
def verify_case(request, pk):
    """Page to verify the authenticity of a SHOFCO case."""
    case = get_object_or_404(Case, pk=pk)
    return render(request, 'cases/verify_case.html', {'case': case})
...
@login_required
def upcoming_cases_by_subcounty(request):
    user = request.user
    today = date.today()

    user_county = None
    if not user.is_superuser:
        user_case = Case.objects.filter(user=user).exclude(county__isnull=True).first()
        if user_case:
            user_county = user_case.county
    ...
    if user_county:
        base_qs = base_qs.filter(county__iexact=user_county)

    subcounties_with_upcoming = (
        base_qs.exclude(case_constituency_name__isnull=True)
        .exclude(case_constituency_name__exact='')
        .values('case_constituency_name')
        .annotate(soonest_date=Min('date_of_court_followup'))
        .order_by('soonest_date')
    )

    cases_by_subcounty = {}
    for entry in subcounties_with_upcoming:
        subcounty = entry['case_constituency_name']
        cases = (
            base_qs.filter(case_constituency_name=subcounty)
            .order_by('date_of_court_followup')[:5]
        )
        cases_by_subcounty[subcounty] = cases

    return render(
        request,
        'cases/upcoming_cases_by_subcounty.html',
        {
            'cases_by_subcounty': cases_by_subcounty,
...
def case_dashboard(request):
    ...
    age_group_data = {'labels': [a['age_group'] or 'Unknown' for a in age_group],
    ...
    referral_data = {'labels': list(referral_map.keys()),
    ...
    county_data_json = {'labels': [entry['county'] or 'Unknown' for entry in county_data],
    ...
    subcounty_data_json = {'labels': [entry['case_constituency_name'] or 'Unknown' for entry in subcounty_data],
                           'data': [entry['total'] for entry in subcounty_data]}

    time_series = all_cases.annotate(month=TruncMonth('date_of_case_reporting')) \
        .values('month').annotate(count=Count('id')).order_by('month')
    time_series_json = {'labels': [entry['month'].strftime('%b %Y') for entry in time_series if entry['month']],
    ...
    assault_stacked = {'labels': counties,
                       'datasets': [{'label': assault, 'data': [assault_data[assault].get(c, 0) for c in counties]}
                                    for assault in sorted(assault_types)]}

    referral_stacked = {'labels': counties,
                        'datasets': [{'label': label,
                                      'data': [all_cases.filter(county=c, **{field: True}).count() for c in counties]}
